...
def Creat_satellite(numOrbitPlanes=6, numSatsPerPlane=11, hight=781, Inclination=86.4, name='Sat'):
    ...
    for orbitPlaneNum in range(numOrbitPlanes):  # RAAN in degrees
        ...
        for satNum in range(numSatsPerPlane):  # trueAnomaly in degrees
            ...
            RAAN = 360 / (numOrbitPlanes * 2) * orbitPlaneNum
...
# Create new scenario
print("Creating Iridium Constellation Scenario...")
if not Read_Scenario:
...
# 如果不是读取当前场景，即首次创建场景
numOrbitPlanes = 6  # 轨道平面数
numSatsPerPlane = 11  # 每个轨道平面的卫星数
hight = 781  # 卫星高度
Inclination = 86.4  # 轨道倾角

if not Read_Scenario:
    Creat_satellite(numOrbitPlanes=numOrbitPlanes, numSatsPerPlane=numSatsPerPlane,
                    hight=hight, Inclination=Inclination)  # Iridium
    # Kuiper
